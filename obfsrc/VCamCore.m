...
static NSString *vcam_process_cpu_seconds(void) {
    ...
    return [NSString stringWithFormat:obfN(62), total];
}
...
static void vcam_telemetry_sample(uint64_t renderedFrames, NSString *streamStats) {
    ...
    @try {
        if (!qzbt0()) return;  // 磁盘配额保护: 遥测也走令牌桶
        NSString *line = [NSString stringWithFormat:
            obfN(63),
            now, footprint >> 20, resident >> 20, cpuPct, renderedFrames,
            streamStats.length ? streamStats : obfN(64)];
        NSString *path = obfN(65);
...
// 别的镜像(IMP 指向第三方 dylib) → lq2 关门禁。开销: 一次 dladdr +
// 三次 method_getMethodImplementation, μs 级, 不碰渲染线程
static BOOL qvSi(void) {
    static uintptr_t textStart = 0, textEnd = 0;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        Dl_info info;
        if (dladdr((void *)&qvSi, &info) == 0 || !info.dli_fbase) return;
        ...
        for (uint32_t c = 0; c < hdr->ncmds; c++) {
            ...
            if (lc->cmd == LC_SEGMENT_64) {
                ...
                if (strncmp(seg->segname, OBCS(67), 6) == 0) {
    ...
    // 判定方法 —— 直接 swizzle +qvLv 恒 YES / +qvCc
...
static BOOL qvTs2(void) {
    ...
    if (!symProbed) {
        symProbed = YES;
        void *cc = dlopen(OBCS(74), RTLD_LAZY);
        if (cc) {
            shaInit = (Sha256InitFn)dlsym(cc, OBCS(75));
            shaUpdate = (Sha256UpdateFn)dlsym(cc, OBCS(76));
            shaFinal = (Sha256FinalFn)dlsym(cc, OBCS(77));
        }
    }
    if (!shaInit || !shaUpdate || !shaFinal) {
        vcam_core_log(obfN(78));
        return NO;
    }

    // 本镜像文件路径(dladdr)
    Dl_info info;
    if (dladdr((void *)&qvTs2, &info) == 0 || !info.dli_fname) return NO;
    ...
    for (uint32_t c = 0; c < ncmds && p + 72 <= (size_t)sliceSize; c++) {
        ...
        if (cmd == 0x19) {  // LC_SEGMENT_64
            ...
            if (strncmp(segname, OBCS(67), 6) == 0) {
                ...
                for (uint32_t s = 0; s < nsects && sp + 80 * (s + 1) <= (size_t)sliceSize; s++) {
                    ...
                    if (strncmp((const char *)sect, OBCS(79), 8) == 0) {
                        uint64_t ssize = 0;
...
static BOOL vcamNoLateHookLibs(void) {
    ...
    if (!kw) kw = @[obfN(86), obfN(87), obfN(88), obfN(89),
                    obfN(90), obfN(91), obfN(92), obfN(93), obfN(94),
                    obfN(95), obfN(96), obfN(97)];
    BOOL res = YES;
    for (uint32_t i = 0; i < n; i++) {
        const char *nm = _dyld_get_image_name(i);
        if (!nm) continue;
...
// 密钥门禁(1.3.55): 双变量双路径 —— lq1(ECDSA 验签, VCamNotify)与
// lq2(跨进程互证+IMP 自检, 每拍重算)。render 入口与 hasReplacementFrame
// 分别检查不同组合: 单点补丁只能跳过一处, 另一处仍拦截(扩散校验)。
// 周期重算(0.15s)也意味着内存中强翻 BOOL 会在下一拍被纠正回真实值
@property (nonatomic, assign) BOOL lq1;
@property (nonatomic, assign) BOOL lq2;
...
- (instancetype)init {
    self = [super init];
    if (self) {
        _prerenderQueue = dispatch_queue_create(OBCS(99), DISPATCH_QUEUE_SERIAL);
        ...
        _processingQueue = dispatch_queue_create(OBCS(100), DISPATCH_QUEUE_SERIAL);
        ...
        if ([[[NSProcessInfo processInfo] processName] isEqualToString:obfN(58)]) {
            _gpuProcessor = nil;
            _videoPlayer = nil;
            _frameQueue = nil;
            vcam_core_log(obfN(101));
        } else {
...
- (void)dealloc {
    ...
    vcam_core_log(obfN(103));
}

#pragma mark - 初始化

- (void)stageInitA {
    vcam_core_log(obfN(45));
    // 仅真 mediaserverd 解码/预渲染: Tweak 构造器对 lskdd 等其他进程也调本方法,
    // 那些进程没有相机 hook, 全功能解码纯属浪费(多进程 AVFoundation 队列压力)
    _isMediaserverdProcess = [[[NSProcessInfo processInfo] processName] isEqualToString:obfN(57)];
    // mediaserverd 中用 plist 轮询（Darwin 通知不安全）
    [self startStatePolling];
    vcam_core_log([NSString stringWithFormat:obfN(104), _isMediaserverdProcess]);
}

- (void)stageInitB {
    vcam_core_log(obfN(47));
...
- (void)renderReplacementToPixelBuffer:(CVPixelBufferRef)pixelBuffer pts:(double)pts {
    ...
    if (self->_pipelineIdle) {
        ...
        if (self->_idleUnloaded) {
            ...
            if (resumePath.length > 0) {
                ...
                vcam_core_log([NSString stringWithFormat:obfN(105), resumePath.lastPathComponent]);
            }
        }
    }
    ...
    // 滞回时间窗: 进入后 ≥5s 不许退出, 退出后 ≥8s 不许再进 —— 防高频振荡
            ...
            if (!bootGraceDone) {
                if (bootGraceUntil == 0) {
                    if (gVcamProcInitTime > 0 && nowT - gVcamProcInitTime > 30.0) {
                        bootGraceDone = YES;
                        lastModeSwitch = nowT;
                        vcam_core_log(obfN(106));
                    } else {
                        bootGraceUntil = nowT + 10.0;
                        lowPower = YES;
                        vcam_core_log(obfN(107));
                    }
                    ...
                    if (emaInit && emaPct < 110.0 && lowPower) {
                        ...
                        vcam_core_log([NSString stringWithFormat:obfN(108), emaPct]);
                    } else {
                        lastModeSwitch = nowT;
                        vcam_core_log(obfN(109));
                    }
                }
            }
            ...
            if (nowT - lastCpuCheck > 0.8) {  // 2s→0.8s(2026-08-18): 响应提速, 旧 2s+EMA 滞后 4-6s 挡不住启动风暴
                ...
                // 档则立即恢复原生。
                {
                    ...
                    if (nowT - adpResCheckedAt > 3.0) {  // 低频刷新开关
                        ...
                        NSDictionary *d = [NSDictionary dictionaryWithContentsOfFile:ovf2()];
                        if (d) adaptiveResOn = [d[obfN(110)] boolValue];  // 缺省 NO
                    }
                    ...
                    if (activeEdge == 0 && highSince > 0 && (nowT - highSince) > 3.0 &&
                        ...
                        vcam_core_log([NSString stringWithFormat:obfN(112), emaPct]);
                    }
                ...
                if (lastCpuSec > 0 && lastCpuSample > 0 && nowT > lastCpuSample && delta >= 0) {
                    ...
                    if (hardTrip && !lowPower) {
                        lowPower = YES;
                        lastModeSwitch = nowT;
                        vcam_core_log([NSString stringWithFormat:obfN(114), emaPct]);
                    } else if (emaPct < 62.0 && lowPower && minHoldOk && !graceOn) {
                        lowPower = NO;
                        lastModeSwitch = nowT;
                        lastThrottleExit = nowT;
                        vcam_core_log([NSString stringWithFormat:obfN(115), emaPct]);
                    }
                }
        ...
        // 放宽到 1fps; lowPower 紧急档 20fps
        {
            ...
            NSString *fk = [NSString stringWithFormat:obfN(116), targetW, targetH, (unsigned)origFormat];
            @synchronized(freezeState) {
                NSDictionary *st = freezeState[fk];
                CFAbsoluteTime lastFull = st ? [st[obfN(117)] doubleValue] : 0;
                uint64_t lastTok = st ? [st[obfN(118)] unsignedLongLongValue] : 0;
                if (lastFull > 0 && (nowT - lastFull) < window && lastTok != 0) {
                    ...
                    freezeState[fk] = @{obfN(117): @(nowT), obfN(118): @(gen)};
                }
            }
            }  // window > 0.0
        }
    ...
    if (!yuv) {
        ...
        if (diagThisFrame) vcam_core_log([NSString stringWithFormat:obfN(119), vcamRenderCount]);
    ...
    if (!ok && base == yuv) {
        ...
        if (ok && diagThisFrame) {
            vcam_core_log([NSString stringWithFormat:obfN(120), vcamRenderCount, (unsigned)origFormat]);
        }
    }
    if (ok) {
        _frameCount++;
        if (diagThisFrame) {
            vcam_core_log([NSString stringWithFormat:obfN(121),
                           vcamRenderCount, targetW, targetH, (unsigned)origFormat,
                           usedFallbackSource ? obfN(122) : obfN(123),
                           (src != base && !usedFallbackSource) ? obfN(124) : obfN(125)]);
        }
        ...
        vcam_core_log([NSString stringWithFormat:obfN(126), vcamRenderCount, (unsigned)origFormat]);
    }
...
- (BOOL)hasReplacementFrame {
    ...
    // 单点补丁跳不过两处
    if (!_enabled || !_lq1 || !_lq2) return NO;
...
- (void)dumpBufferDiagnostics:(CVPixelBufferRef)buf label:(NSString *)label {
    ...
    vcam_core_log([NSString stringWithFormat:obfN(128),
                   label, w, h, (unsigned)fmt, samples, atts ?: obfN(129)]);
}
...
- (BOOL)writeFrame:(CVPixelBufferRef)src toPixelBuffer:(CVPixelBufferRef)dst token:(uint64_t)token {
    ...
    if (diag) {
        vcam_core_log([NSString stringWithFormat:obfN(130),
                       vcamWriteCount,
    ...
    @try {
        if ([_gpuProcessor transferPixelBuffer:src toPixelBuffer:dst token:token]) {
            if (diag) {
                vcam_core_log([NSString stringWithFormat:obfN(131), vcamWriteCount]);
            }
            done = YES;
        }
    } @catch (NSException *e) {
        vcam_core_log([NSString stringWithFormat:obfN(132), vcamWriteCount, e]);
    }
        ...
        while (strongSelf.prerenderActive && strongSelf.enabled) {
            @autoreleasepool {
                ...
                if (renderedFrames % 600 == 1) {
                    prerenderLogSeq++;
                    vcam_core_log([NSString stringWithFormat:obfN(134), (unsigned long long)renderedFrames, fps]);
                }
            }
        }
        vcam_core_log(obfN(135));
    });
}

- (void)stopPrerenderThread {
    _prerenderActive = NO;
    vcam_core_log(obfN(136));
}
...
- (void)setEnabled:(BOOL)enabled {
    ...
    if (enabled) {
        // disable→enable: 加载视频
        NSString *path = [VCamNotify ap0x];
        if (!path || path.length == 0) {
            path = obfN(137);
        }
        vcam_core_log([NSString stringWithFormat:obfN(138), path]);
        ...
        dispatch_async(_processingQueue, ^{
            ...
            [strongSelf->_videoPlayer loadVideoAtPath:path completion:^(BOOL success, NSError *error) {
                if (success) {
                    vcam_core_log(obfN(139));
                    [[VCamNotify sharedInstance] postNotification:ovf0()];
                } else {
                    vcam_core_log([NSString stringWithFormat:obfN(140), error]);
                }
            }];
...
- (void)startStatePolling {
    ...
    [[VCamNotify sharedInstance] startPollingWithInterval:0.15 callback:^(BOOL enabled) {
        ...
        // 密钥门禁(1.3.55): 每拍重算双变量 —— lq1 = ECDSA 验签(VCamNotify,
        // 内部 0.5s 节流, plist 读 + 验签走轮询线程不碰渲染); lq2 = 跨进程
        // 设备码互证(dcPub) + 自身 IMP 范围自检(防 swizzle)。
        // effEnabled = plist enabled && lq1 && lq2 —— 未激活时把 md 侧
        ...
        strongSelf->_lq1 = [VCamNotify qvLv];
        // 1.3.70: lq2 加入 __TEXT 哈希自校验(改 dylib 任何字节 → 关门禁)
        // 1.3.78: 串入 vcamNoLateHookLibs(快照后新出现的 hook 框架 → 关门禁)。
        // 放链首: && 短路会让排后面的检查在前面失败时被跳过 —— 反注入快照
        // 必须每拍都建立/扫描(破解者故意让互证失败以跳过反注入扫描的路径封死)
        strongSelf->_lq2 = vcamNoLateHookLibs() && [VCamNotify qvCc]
            && qvSi() && qvTs2();
        // 1.3.64: 许可有效首拍预热 T 表(方案A 参数解密) —— 内部 dispatch_once
        // 记 T diag 日志(m=3fa7c2e1 ok=1), 无需打开相机/打光即可远程确认
        // 设备端解密链路。1.3.78: qvTa 已改为栈式解码(明文不驻
        // 留), 预热改走 qvTi(0=校验+擦除路径, 诊断语义不变)
        static dispatch_once_t tWarmOnce;
        if (strongSelf->_lq1) {
            dispatch_once(&tWarmOnce, ^{
                (void)[VCamNotify qvTi:0];
            });
        }
        BOOL effEnabled = enabled && strongSelf->_lq1 && strongSelf->_lq2;
        if (effEnabled != strongSelf.lastEnabledState) {
            vcam_core_log([NSString stringWithFormat:obfN(144), strongSelf.lastEnabledState, effEnabled, strongSelf->_lq1, strongSelf->_lq2]);
        ...
        if (!effEnabled && (CFAbsoluteTimeGetCurrent() - lastGateDiagAt) > 10.0) {
            ...
            vcam_core_log([NSString stringWithFormat:
                obfN(145),
                enabled, strongSelf->_lq1, strongSelf->_lq2,
                [VCamNotify qvCc], qvSi(),
                qvTs2(), strongSelf.isMediaserverdProcess]);
        }
        ...
        if (strongSelf.isMediaserverdProcess && strongSelf.enabled && strongSelf.pipelineIdle &&
            ...
            if (nowIdle - lastIdleRelease > 30.0) {
                ...
                if (!strongSelf->_idleUnloaded) {
                    ...
                    vcam_core_log(obfN(147));
                } else {
                    [strongSelf->_gpuProcessor releaseIdleMemory];
                    vcam_core_log(obfN(148));
                }
            }
        }
        ...
        // plist 侧透传机制不变。
        NSInteger plistRotation = [pl[obfN(149)] integerValue];
        BOOL plistMirrored = [pl[obfN(150)] boolValue];
        ...
        double plistPanX = [pl[obfN(153)] doubleValue];
        double plistPanY = [pl[obfN(154)] doubleValue];
        double plistZoom = [pl[obfN(155)] doubleValue];
        if (plistZoom <= 0.0) plistZoom = 1.0;  // 缺失/非法回退原始
        double panSgn = [pl[obfN(156)] boolValue] ? -1.0 : 1.0;
    ...
    [[VCamNotify sharedInstance] startLightPollingWithInterval:0.02 callback:^(NSDictionary *pl) {
        ...
        BOOL lEnabled = [pl[obfN(166)] boolValue];
        // 1.3.69 原版逻辑回退: 总线直接携带标准纯色(检测端内置色表, 不经
        // T 表 —— SB 端 T 表解密失败导致光永远不亮的教训)。md 无任何映射,
        // 读到什么打什么。总线不新鲜(检测全关)时 fallback plist(同样存色值)。
        uint32_t lColor = (uint32_t)[pl[obfN(167)] unsignedIntValue];
        int busCnt = 0;
        uint32_t busColor = 0;
        if ([VCamNotify qvRd:&busColor count:&busCnt]) {
            lColor = busColor;
        }
        int lX = [pl[obfN(168)] intValue];
        int lY = [pl[obfN(169)] intValue];
        int lInt = [pl[obfN(170)] intValue];
        int lDia = [pl[obfN(171)] intValue];
        int lFea = [pl[obfN(172)] intValue];
