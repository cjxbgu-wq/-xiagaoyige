...
- (instancetype)init {
    ...
    if (self) {
        ...
        @try {
            ...
            CreateDeviceFn createDevice = (CreateDeviceFn)dlsym(RTLD_DEFAULT, OBCS(175));
            if (createDevice) {
                id device = (__bridge id)createDevice();
                BOOL devicePresent = (device != nil);
                vcam_gpu_log([NSString stringWithFormat:obfN(176),
                              devicePresent ? obfN(177) : obfN(178)]);
            }
        } @catch (NSException *e) {
        }
...
- (void)dealloc {
    ...
    if (_bgraTransferSession) {
        InvalidateFunc invalidate = (InvalidateFunc)dlsym(RTLD_DEFAULT, OBCS(180));
    ...
    {
        InvalidateFunc invalidate = (InvalidateFunc)dlsym(RTLD_DEFAULT, OBCS(181));
        if (invalidate) {
...
- (void)setupBGRATransferSession {
    ...
    if (status == noErr) {
        ...
        VTSessionSetProperty(_bgraTransferSession, obfCF(183), obfCF(184));
        vcam_gpu_log(obfN(185));
    } else {
        vcam_gpu_log([NSString stringWithFormat:obfN(186), (int)status]);
    }
}
...
- (void)setupYUVTransferSession {
    ...
    if (status == noErr) {
        VTSessionSetProperty(_yuvTransferSession, obfCF(183), obfCF(184));
        vcam_gpu_log(obfN(187));
    } else {
        vcam_gpu_log([NSString stringWithFormat:obfN(188), (int)status]);
    }
}
...
- (void)setupPrerenderTransferSession {
    ...
    if (status == noErr) {
        VTSessionSetProperty(_prerenderTransferSession, obfCF(183), obfCF(184));
        vcam_gpu_log(obfN(189));
    } else {
        vcam_gpu_log([NSString stringWithFormat:obfN(190), (int)status]);
    }
}
...
- (void)setupPrivateTransferSession {
    ...
    if (status == noErr) {
        VTSessionSetProperty(_privateTransferSession, obfCF(183), obfCF(184));
        vcam_gpu_log(obfN(191));
    } else {
        vcam_gpu_log([NSString stringWithFormat:obfN(192), (int)status]);
    }
}
...
- (void)invalidateTwoStepSessionForKey:(NSString *)key {
    ...
    if (s) {
        void (*invalidate)(VTPixelTransferSessionRef) =
            (void (*)(VTPixelTransferSessionRef))dlsym(RTLD_DEFAULT, OBCS(180));
...
- (void)setupPixelRotationSession {
    ...
    void *vt = dlopen(OBCS(195), RTLD_LAZY | RTLD_GLOBAL);
    void *base = vt ? vt : RTLD_DEFAULT;

    _createRotationSession = (VTPixelRotationSessionCreateFunc)dlsym(base, OBCS(196));
    _transferRotationImage = (VTPixelRotationSessionTransferImageFunc)dlsym(base, OBCS(197));
    ...
    void *rotSym = dlsym(base, OBCS(198));
    _rotationPropertyKey = rotSym ? *(CFStringRef *)rotSym : obfCF(199);
    void *ccwSym = dlsym(base, OBCS(200));
    _rotationCCW90Value = ccwSym ? *(CFStringRef *)ccwSym : obfCF(201);
    void *cwSym = dlsym(base, OBCS(202));
    _rotationCW90Value = cwSym ? *(CFStringRef *)cwSym : obfCF(203);
    void *r180Sym = dlsym(base, OBCS(204));
    _rotation180Value = r180Sym ? *(CFStringRef *)r180Sym : obfCF(205);
    void *flipSym = dlsym(base, OBCS(206));
    _flipHorizontalKey = flipSym ? *(CFStringRef *)flipSym : obfCF(207);

    vcam_gpu_log([NSString stringWithFormat:obfN(208),
    ...
    if (status == noErr) {
        ...
        vcam_gpu_log([NSString stringWithFormat:obfN(210), (int)st2]);
    } else {
        _rotationApiAvailable = NO;
        vcam_gpu_log([NSString stringWithFormat:obfN(211), (int)status]);
    }
}
...
- (CVPixelBufferPoolRef)getOrCreatePoolForWidth:(size_t)width height:(size_t)height format:(OSType)format {
    ...
    @synchronized(self) {
        NSString *key = [NSString stringWithFormat:obfN(116), width, height, (unsigned)format];
        ...
        if (status == noErr && pool) {
            ...
            vcam_gpu_log([NSString stringWithFormat:obfN(212), width, height, (unsigned)format, key]);
            return pool;
        }

        vcam_gpu_log([NSString stringWithFormat:obfN(213), (int)status, width, height]);
        return NULL;
    }
}
...
- (CVPixelBufferRef)getOrCreateBGRABufferWithWidth:(size_t)width height:(size_t)height CF_RETURNS_RETAINED {
    @synchronized(self) {
        ...
        if (pool) {
            ...
            vcam_gpu_log([NSString stringWithFormat:obfN(214), (int)status]);
        }
        ...
        if (status != noErr) {
            vcam_gpu_log([NSString stringWithFormat:obfN(215), (int)status, width, height]);
            return NULL;
        }
        return buffer;
    }
...
- (void)configureWithWidth:(size_t)width height:(size_t)height format:(OSType)format {
    ...
    vcam_gpu_log([NSString stringWithFormat:obfN(216), width, height, fmtStr]);
}
    ...
    @try {
        ...
        if (format == kCVPixelFormatType_32BGRA) {
            ...
            if (outputBuffer) {
                ...
                if (status != noErr) {
                    NSString *fmtStr = [self stringForFormat:format];
                    vcam_gpu_log([NSString stringWithFormat:obfN(217), (int)status, fmtStr]);
        ...
        vcam_gpu_log([NSString stringWithFormat:obfN(218), e]);
        return NULL;
    }
}
...
- (CVPixelBufferRef)rotateAndMirror:(CVPixelBufferRef)input width:(size_t)width height:(size_t)height angle:(int)angle CF_RETURNS_RETAINED {
    ...
    if (status != noErr) {
        vcam_gpu_log([NSString stringWithFormat:obfN(219), (int)status]);
        return NULL;
    }
    ...
    if (status != noErr) {
        vcam_gpu_log([NSString stringWithFormat:obfN(220), (int)status]);
    ...
    if (needRotate && _rotationApiAvailable && _pixelRotationSession && _transferRotationImage) {
        ...
        if (dst) {
            ...
            if (st == noErr) {
                ...
                if (rotFailLogged++ < 2) {
                    vcam_gpu_log([NSString stringWithFormat:obfN(221), (int)st]);
                }
            }
        }
    }
                ...
                if (pieceA) {
                    ...
                    if (!_userTransferSession) {
                        if (VTPixelTransferSessionCreate(kCFAllocatorDefault, &_userTransferSession) == noErr) {
                            VTSessionSetProperty(_userTransferSession, obfCF(183), obfCF(184));
                            VTSessionSetProperty(_userTransferSession, obfCF(193), kCFBooleanTrue);
                            vcam_gpu_log(obfN(222));
                        }
                    }
                    if (_userTransferSession) {
...
- (CVPixelBufferRef)injectLightIntoFrame:(CVPixelBufferRef)input CF_RETURNS_RETAINED {
    ...
    if (!canvas || CVPixelBufferGetWidth(canvas) != W ||
        ...
        vcam_gpu_log([NSString stringWithFormat:obfN(223), W, H, slot]);
    }
...
- (CVPixelBufferRef)adaptiveRotateIfNeeded:(CVPixelBufferRef)src
    ...
    [_rotationRenderLock unlock];
    if (st != noErr) {
        vcam_gpu_log([NSString stringWithFormat:obfN(224),
...
- (CVPixelBufferRef)createBufferWithWidth:(size_t)width height:(size_t)height format:(OSType)format CF_RETURNS_RETAINED {
    ...
    if (status != noErr) {
        vcam_gpu_log([NSString stringWithFormat:obfN(226),
...
- (VTPixelTransferSessionRef)oneStepSessionForKey:(NSString *)key {
    ...
    if (VTPixelTransferSessionCreate(kCFAllocatorDefault, &s) == noErr && s) {
        VTSessionSetProperty(s, obfCF(183), obfCF(184));
        VTSessionSetProperty(s, obfCF(193), kCFBooleanTrue);  // 硬件路径提示
        [_poolDictLock lock];
...
- (VTPixelTransferSessionRef)normalTransferSession {
    if (!gVcamNormalSession) {
        if (VTPixelTransferSessionCreate(kCFAllocatorDefault, &gVcamNormalSession) == noErr) {
            VTSessionSetProperty(gVcamNormalSession, obfCF(183), obfCF(227));
            VTSessionSetProperty(gVcamNormalSession, obfCF(193), kCFBooleanTrue);
            vcam_gpu_log(obfN(228));
        }
    }
...
- (CVPixelBufferRef)cropStagingForRatio:(CVPixelBufferRef)staging dst:(CVPixelBufferRef)dst srcToken:(uint64_t)srcToken {
    ...
    if (!slot) {
        ...
        vcam_gpu_log([NSString stringWithFormat:obfN(229), cw, ch, idx]);
    }
    slot->lastUse = now;
...
- (CVPixelBufferRef)yuvStagingForSrc:(CVPixelBufferRef)s2src
    ...
    if (!slot) {
        ...
        if (cc != noErr || !nb) {
            ...
            vcam_gpu_log([NSString stringWithFormat:obfN(230), (int)cc, mw, mh, (unsigned)dstFormat]);
            return NULL;
        }
        ...
        vcam_gpu_log([NSString stringWithFormat:obfN(231), mw, mh, (unsigned)dstFormat, idx]);
    }
    slot->lastUse = now;
    ...
    if (st != noErr) {
        ...
        vcam_gpu_log([NSString stringWithFormat:obfN(232), (int)st, (unsigned)dstFormat]);
        return NULL;
    }
    slot->token = srcToken;
    return slot->staging;
...
- (BOOL)transferPixelBuffer:(CVPixelBufferRef)src toPixelBuffer:(CVPixelBufferRef)dst token:(uint64_t)token {
    ...
    if (!isBgraLane && !isYuvLane) {
        ...
        if (!_twoStepS1Session) {
            if (VTPixelTransferSessionCreate(kCFAllocatorDefault, &_twoStepS1Session) == noErr) {
                VTSessionSetProperty(_twoStepS1Session, obfCF(183), obfCF(184));
                VTSessionSetProperty(_twoStepS1Session, obfCF(193), kCFBooleanTrue);
                vcam_gpu_log(obfN(234));
            }
        }
        if (!_privateTransferSession) {
            if (VTPixelTransferSessionCreate(kCFAllocatorDefault, &_privateTransferSession) == noErr) {
                VTSessionSetProperty(_privateTransferSession, obfCF(183), obfCF(184));
                VTSessionSetProperty(_privateTransferSession, obfCF(193), kCFBooleanTrue);
                vcam_gpu_log(obfN(235));
            }
        }
            ...
            for (int i = 0; i < kVcamLaneStagingMax; i++) {
                if (!gVcamLaneStaging[i].staging) {
                    ...
                    if (CVPixelBufferCreate(kCFAllocatorDefault, srcW, srcH,
                        ...
                        gVcamLaneStaging[i].token = 0;
                        slot = &gVcamLaneStaging[i];
                        vcam_gpu_log([NSString stringWithFormat:obfN(241), srcW, srcH, i]);
                    }
                    break;
                }
            }
        ...
        if (_twoStepS1Session && _privateTransferSession && slot) {
            ...
            if (token != 0 && slot->token == token) {
                ...
                if (st1 == noErr) {
                    ...
                    vcam_gpu_log([NSString stringWithFormat:obfN(242), (int)st1]);
                }
            }
                ...
                if (dstPx > 4 * srcPx && vcamYuvSplitState((uint32_t)dstFormat) >= 0) {
                    ...
                    if (yuvMid) {
                        ...
                        if (st2 != noErr) {
                            ...
                            vcam_gpu_log([NSString stringWithFormat:obfN(243), (int)st2, (unsigned)dstFormat]);
                        }
                    }
                }
                if (st2 != noErr) {
        ...
        // 熔断(30s 自动恢复, 同步刷新 memo)
        {
            ...
            if (fails >= 2) {
                ...
                vcam_gpu_log([NSString stringWithFormat:obfN(244), (unsigned)dstFormat]);
            }
        }
        return NO;
    }
    ...
    // 熔断(30s 自动恢复)
    {
        ...
        if (fails >= 2) {
            @synchronized(self) { _laneDisabled[@(dstFormat)] = @YES; }
            vcamLaneMemoInvalidate(dstFormat, YES);
            vcam_gpu_log([NSString stringWithFormat:obfN(246),
...
- (void)touchStreamKeyLRU:(NSString *)key {
    ...
    @synchronized(self) {
        ...
        while (_streamKeyOrder.count > kVcamMaxStreamKeys) {
            ...
            vcam_gpu_log([NSString stringWithFormat:obfN(248), old, (unsigned long)_streamKeyOrder.count]);
        }
    }
}
...
- (void)releaseIdleMemory {
    ...
    if (freed > 0) {
        vcam_gpu_log([NSString stringWithFormat:obfN(249), (unsigned long)freed]);
    }
}
...
- (void)releaseHeavyBuffersForIdle {
    ...
    [_laneLockPrivate unlock];
    vcam_gpu_log([NSString stringWithFormat:obfN(250), (unsigned long)released]);
}
...
- (NSString *)takeStreamStats {
    ...
    for (int i = 0; i < kVcamStatSlots; i++) {
        ...
        [parts addObject:[NSString stringWithFormat:obfN(251),
                          s->w, s->h, s->fmt, s->renders, mb, s1Avg, s2Avg]];
    }
    if (parts.count > 0) out = [parts componentsJoinedByString:obfN(252)];
    ...
    @try {
        ...
        [_ciGPUContext render:out toCVPixelBuffer:dst];
        {
            ...
            if (gpuRenderCount % 300 == 0) {
                vcam_gpu_log([NSString stringWithFormat:obfN(253),
        ...
        vcam_gpu_log([NSString stringWithFormat:obfN(254), e]);
        return NO;
    }
}
...
- (BOOL)twoStepTransferLocked:(CVPixelBufferRef)src toPixelBuffer:(CVPixelBufferRef)dst
    ...
    if (![_resultBlitDisabledPool[poolKey] boolValue]) {
        ...
        if (cacheBuf && ct && [ct unsignedLongLongValue] == token) {
            ...
            if (blitS) {
                ...
                vcam_gpu_log([NSString stringWithFormat:obfN(255), (int)stB, poolKey]);
            }
        }
    }
    ...
    NSString *ratioKey = [NSString stringWithFormat:obfN(256),
    ...
    CVPixelBufferRef groupStaging = NULL;
    {
        ...
        if (!groupStaging || needPx > havePx) {
            ...
            if (bigger) {
                ...
                vcam_gpu_log([NSString stringWithFormat:obfN(257), ratioKey, dstW, dstH]);
            }
        ...
        if (!gt || [gt unsignedLongLongValue] != token) {
            ...
            if (gSession) {
                ...
                if (st1 == noErr) {
                    ...
                    vcam_gpu_log([NSString stringWithFormat:obfN(258), (int)st1, ratioKey]);
                }
            }
        } else {
    ...
    if (repeatConsume && ![_resultBlitDisabledPool[poolKey] boolValue] && !_resultCachePool[poolKey]) {
        ...
        if (cacheBuf) {
            ...
            if (st2a == noErr) {
                ...
                _resultBlitDisabledPool[poolKey] = @YES;
                CVPixelBufferRelease(cacheBuf);
                vcam_gpu_log([NSString stringWithFormat:obfN(259), (int)st2b, poolKey]);
            } else {
                CVPixelBufferRelease(cacheBuf);
            }
        } else {
...
- (BOOL)legacyTwoStepTransferLocked:(CVPixelBufferRef)src toPixelBuffer:(CVPixelBufferRef)dst
    ...
    if (!staging) {
        ...
        vcam_gpu_log([NSString stringWithFormat:obfN(260), poolKey,
    ...
    if (!stagingFresh) {
        ...
        if (st1 != noErr &&
            ...
            vcam_gpu_log([NSString stringWithFormat:obfN(261),
                          (int)st1, poolKey]);
        ...
        if (st1 != noErr) {
            ...
            if (fails >= 2) {
                ...
                vcam_gpu_log([NSString stringWithFormat:obfN(262), poolKey, (int)st1]);
            } else {
                vcam_gpu_log([NSString stringWithFormat:obfN(263), (int)st1, poolKey, (long)fails]);
            }
            return NO;
        }
    ...
    if (st2 != noErr) {
        ...
        if (fails >= 2) {
            ...
            vcam_gpu_log([NSString stringWithFormat:obfN(264), poolKey]);
        } else {
            vcam_gpu_log([NSString stringWithFormat:obfN(265), (int)st2, poolKey, (long)fails]);
        }
        return NO;
    }
