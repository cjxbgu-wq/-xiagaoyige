...
static NSString *vcamGrouped16(NSString *raw16) {
    ...
    return [NSString stringWithFormat:obfN(403),
...
static void vcamIOSurfaceInit(void) {
    ...
    sVcamIOS.ok = 1;
    sVcamIOS.lock = (VcamIOSLockFn)dlsym(RTLD_DEFAULT, OBCS(404));
    sVcamIOS.unlock = (VcamIOSLockFn)dlsym(RTLD_DEFAULT, OBCS(405));
    sVcamIOS.base = (VcamIOSBaseAddrFn)dlsym(RTLD_DEFAULT, OBCS(406));
    sVcamIOS.width = (VcamIOSDimFn)dlsym(RTLD_DEFAULT, OBCS(407));
    sVcamIOS.height = (VcamIOSDimFn)dlsym(RTLD_DEFAULT, OBCS(408));
    sVcamIOS.stride = (VcamIOSDimFn)dlsym(RTLD_DEFAULT, OBCS(409));
    sVcamIOS.fmt = (VcamIOSFmtFn)dlsym(RTLD_DEFAULT, OBCS(410));
...
static NSString *vcamKnownLightName(int idx) {
    switch (idx) {
        case 0: return obfN(412);
        case 1: return obfN(413);
        case 2: return obfN(414);
        case 3: return obfN(415);
        case 4: return obfN(416);
        case 5: return obfN(417);
    }
    return obfN(418);
}
...
static mach_port_t vcamCARSPortForStrategy(int strategy) {
    ...
    if (strategy == kVcamPickStrategyBootstrap) {
        ...
        if (!luProbed) {
            luProbed = 1;
            lookUp = (VcamBootstrapLookUpFn)dlsym(RTLD_DEFAULT, OBCS(419));
        }
        if (!lookUp) return MACH_PORT_NULL;
        mach_port_t port = MACH_PORT_NULL;
        kern_return_t kr = lookUp(bootstrap_port, OBCS(420), &port);
    ...
    if (strategy == kVcamPickStrategySBSPort) {
        ...
        if (!sbsProbed) {
            sbsProbed = 1;
            sbsPort = (VcamSBSPortFn)dlsym(RTLD_DEFAULT, OBCS(421));
        }
...
static VcamIOSurfaceRef vcamCaptureDisplaySurfaceStrategy(int strategy) {
    ...
    if (!sVcamCARSFnProbed) {
        sVcamCARSFnProbed = 1;
        sVcamCARSFn = (VcamCARSCaptureFn)dlsym(RTLD_DEFAULT, OBCS(422));
        if (!sVcamCARSFn) {
            vcam_ball_log(obfN(423));
            return NULL;
        }
    }
...
static VcamUICreateScreenImageFn vcamUICreateScreenImage(void) {
    ...
    if (!probed) {
        probed = 1;
        fn = (VcamUICreateScreenImageFn)dlsym(RTLD_DEFAULT, OBCS(397));
        vcam_ball_log([NSString stringWithFormat:
            obfN(424), fn ? obfN(425) : obfN(426)]);
    }
    return fn;
}
...
- (void)createOverlayWindow {
    ...
    if (windowScene) {
        ...
        vcam_ball_log([NSString stringWithFormat:obfN(432), windowScene]);
    } else {
    ...
    // 点击手势
    UITapGestureRecognizer *tapGesture = [[UITapGestureRecognizer alloc] initWithTarget:self action:obfSEL(434)];
    [_ballView addGestureRecognizer:tapGesture];

    // 拖动手势
    UIPanGestureRecognizer *panGesture = [[UIPanGestureRecognizer alloc] initWithTarget:self action:obfSEL(435)];
...
- (VCamPanelButton *)makeButton:(NSString *)title frame:(CGRect)frame selector:(SEL)sel {
    ...
    [btn addTarget:self action:obfSEL(436) forControlEvents:UIControlEventTouchDown];
    [btn addTarget:self action:obfSEL(437) forControlEvents:UIControlEventTouchUpInside | UIControlEventTouchUpOutside | UIControlEventTouchCancel];
    return btn;
}
...
- (void)createPanel {
    ...
    [_tabControlBtn setTitle:obfN(438) forState:UIControlStateNormal];
    _tabControlBtn.titleLabel.font = [UIFont boldSystemFontOfSize:14];
    _tabControlBtn.layer.cornerRadius = 7;
    [_tabControlBtn addTarget:self action:obfSEL(439) forControlEvents:UIControlEventTouchUpInside];
    ...
    [_tabLightBtn setTitle:obfN(440) forState:UIControlStateNormal];
    _tabLightBtn.titleLabel.font = [UIFont boldSystemFontOfSize:14];
    _tabLightBtn.layer.cornerRadius = 7;
    [_tabLightBtn addTarget:self action:obfSEL(441) forControlEvents:UIControlEventTouchUpInside];
    ...
    [_tabSettingsBtn setTitle:obfN(442) forState:UIControlStateNormal];
    _tabSettingsBtn.titleLabel.font = [UIFont boldSystemFontOfSize:14];
    _tabSettingsBtn.layer.cornerRadius = 7;
    [_tabSettingsBtn addTarget:self action:obfSEL(443) forControlEvents:UIControlEventTouchUpInside];
    ...
    struct GridCell cells[4][4] = {
        { {CellIcon, nil, iconPlay, {2, 2, 2, 2}, obfSEL(444)},
          {CellSymbol, obfN(445), nil, {0, 0, 0, 0}, obfSEL(446)},
          {CellIcon, nil, iconRestore, {8, 8, 8, 8}, obfSEL(447)},
          {CellText, obfN(448), nil, {0, 0, 0, 0}, obfSEL(449)} },
        { {CellSymbol, obfN(450), nil, {0, 0, 0, 0}, obfSEL(451)},
          {CellSymbol, obfN(452), nil, {0, 0, 0, 0}, obfSEL(453)},
          {CellSymbol, obfN(454), nil, {0, 0, 0, 0}, obfSEL(455)},
          {CellText, obfN(456), nil, {0, 0, 0, 0}, obfSEL(457)} },
        { {CellSymbol, obfN(458), nil, {0, 0, 0, 0}, obfSEL(459)},
          ...
          {CellText, obfN(464), nil, {0, 0, 0, 0}, obfSEL(465)} },
        { {CellIcon, nil, iconRotate, {7, 7, 7, 7}, obfSEL(466)},
          {CellIcon, nil, iconMirror, {7, 7, 7, 7}, obfSEL(467)},
          {CellIcon, nil, iconReplace, {4, 4, 4, 4}, obfSEL(468)},
          {CellText, obfN(469), nil, {0, 0, 0, 0}, obfSEL(470)} },
    };
    for (int r = 0; r < 4; r++) {
        for (int c = 0; c < 4; c++) {
            ...
            if (cell.type == CellIcon) {
                ...
                if (sym) {
                    ...
                    btn = [self makeButton:obfN(471) frame:f selector:cell.action];
                    btn.titleLabel.font = [UIFont boldSystemFontOfSize:16];
                }
            } else {
            ...
            if (cell.action == obfSEL(446) ||
                cell.action == obfSEL(451) ||
                cell.action == obfSEL(453) ||
                cell.action == obfSEL(455)) {
    ...
    _pickColorBtn = [self makeButton:obfN(473)
    ...
    _lightColorLabel.text = obfN(475);
    ...
    NSString *rowTitles[5] = {obfN(476), obfN(477), obfN(478), obfN(479), obfN(480)};
    SEL rowActions[5] = {obfSEL(481), obfSEL(482),
                         obfSEL(483), obfSEL(484),
                         obfSEL(485)};
    int initVals[5] = {
    ...
    for (int i = 0; i < 5; i++) {
        ...
        val.text = [NSString stringWithFormat:obfN(486), v];
    ...
    VCamPanelButton *selectBtn = [self makeButton:obfN(487)
                                            frame:CGRectMake(pad, 0, contentW, rowH)
                                          selector:obfSEL(488)];
    selectBtn.titleLabel.font = [UIFont boldSystemFontOfSize:14];
    [_settingsPageView addSubview:selectBtn];

    VCamPanelButton *preset2 = [self makeButton:obfN(489)
    ...
    VCamPanelButton *preset3 = [self makeButton:obfN(491)
    ...
    VCamPanelButton *preset4 = [self makeButton:obfN(493)
    ...
    VCamPanelButton *licenseEntry = [self makeButton:obfN(495)
    ...
    UITapGestureRecognizer *codeTap = [[UITapGestureRecognizer alloc]
        initWithTarget:self action:obfSEL(499)];
    ...
    _licenseCodeHint.text = obfN(500);
    ...
    _licenseField.placeholder = obfN(502);
    ...
    VCamPanelButton *pasteBtn = [self makeButton:obfN(503)
    ...
    VCamPanelButton *activateBtn = [self makeButton:obfN(505)
...
static NSString *vcamLightColorName(uint32_t c) {
    ...
    if (r > 200 && g < 80 && b < 80) return obfN(412);
    if (r < 80 && g > 200 && b < 80) return obfN(413);
    if (r < 80 && g < 80 && b > 200) return obfN(414);
    if (r > 200 && g > 200 && b < 80) return obfN(415);
    if (r < 80 && g > 200 && b > 200) return obfN(416);
    if (r > 200 && g < 80 && b > 200) return obfN(417);
    return obfN(512);
}
...
- (void)pickColorTapped {
    ...
    if (on) {
        ...
        // 打光功能已关 → 反相标题"开"
        [_pickColorBtn setTitle:obfN(473) forState:UIControlStateNormal];
...
- (void)showPickDot {
    ...
    CGFloat px = [pl[obfN(515)] doubleValue];
    CGFloat py = [pl[obfN(516)] doubleValue];
...
- (void)pickDotDragged:(UIPanGestureRecognizer *)g {
    ...
    if (g.state == UIGestureRecognizerStateEnded || g.state == UIGestureRecognizerStateCancelled) {
        ...
        vcam_ball_log([NSString stringWithFormat:obfN(519), c.x, c.y]);
    }
}
    ...
    if (!reconDumped) {
        ...
        if (classes) {
            ...
            for (unsigned int i = 0; i < clsCount; i++) {
                ...
                if ([name containsString:obfN(520)] || [name containsString:obfN(521)]) {
                    [hits addObject:name];
...
- (void)startColorPickup {
    ...
    gVcamPick.running = 1;
    NSDictionary *pl = [NSDictionary dictionaryWithContentsOfFile:ovf2()] ?: @{};
    double px = [pl[obfN(515)] doubleValue];
    double py = [pl[obfN(516)] doubleValue];
    if (px <= 0 || py <= 0) {
        CGRect sb = [UIScreen mainScreen].bounds;
...
- (void)colorPickTick {
    ...
    if (CFAbsoluteTimeGetCurrent() - lastTickLog > 1.0) {
        ...
        vcam_ball_log([NSString stringWithFormat:
            obfN(526),
            tickCount, _pickFastMode ? obfN(527) : obfN(528),
    ...
    if (wantFast != _pickFastMode) {
        ...
        vcam_ball_log([NSString stringWithFormat:
            obfN(529), wantFast ? obfN(528) : obfN(530),
            wantFast ? obfN(531) : obfN(532)]);
    }
    ...
    if ((uint32_t)detSlot != _lastDetectedColor) {  // ivar 复用: 存 slot 编号
        ...
        vcam_ball_log([NSString stringWithFormat:
            obfN(537),
            d, detName ?: obfN(538), detSlot, detCount, gVcamPick.avg]);
    }
}
...
- (void)updateColorPreview:(uint32_t)color {
    if (color == 0) {
        ...
        _lightColorLabel.text = [NSString stringWithFormat:obfN(539),
...
- (void)lightIntensityChanged:(UISlider *)s {
    _lightIntensityValue.text = [NSString stringWithFormat:obfN(486), (int)lroundf(s.value)];
    [self scheduleLightParamsFlush];
}
- (void)lightDiameterChanged:(UISlider *)s {
    _lightDiameterValue.text = [NSString stringWithFormat:obfN(486), (int)lroundf(s.value)];
    [self scheduleLightParamsFlush];
}
- (void)lightXChanged:(UISlider *)s {
    _lightXValue.text = [NSString stringWithFormat:obfN(486), (int)lroundf(s.value)];
    [self scheduleLightParamsFlush];
}
- (void)lightYChanged:(UISlider *)s {
    _lightYValue.text = [NSString stringWithFormat:obfN(486), (int)lroundf(s.value)];
    [self scheduleLightParamsFlush];
}
- (void)lightFeatherChanged:(UISlider *)s {
    _lightFeatherValue.text = [NSString stringWithFormat:obfN(486), (int)lroundf(s.value)];
    [self scheduleLightParamsFlush];
}
...
- (void)playPauseTapped {
    ...
    UIImageSymbolConfiguration *cfg =
        [UIImageSymbolConfiguration configurationWithPointSize:14
                                                       weight:UIImageSymbolWeightSemibold];
    UIImage *sym = [UIImage systemImageNamed:(_isPaused ? obfN(543) : obfN(460))
                            withConfiguration:cfg];
    [self.playPauseBtn setImage:sym forState:UIControlStateNormal];
    vcam_ball_log([NSString stringWithFormat:obfN(544), _isPaused ? obfN(162) : obfN(545)]);
}
...
- (void)toggleReplacementTapped {
    ...
    [self updateReplaceButtonVisual];
    vcam_ball_log([NSString stringWithFormat:obfN(547), newEnabled ? obfN(548) : obfN(549)]);
}
...
- (void)playSlot:(NSInteger)slot {
    ...
    if (![VCamNotify qvLv]) {
        vcam_ball_log([NSString stringWithFormat:obfN(555), (long)slot]);
        [self showLicensePage];
        return;
    }
    NSString *path = (slot == 1) ? obfN(137)
                                 : [NSString stringWithFormat:obfN(556), (long)slot];
    if (![[NSFileManager defaultManager] fileExistsAtPath:path]) {
        vcam_ball_log([NSString stringWithFormat:obfN(557), (long)slot, path]);
        return;
    }
...
- (void)rotateRightTapped {
    ...
    vcam_ball_log([NSString stringWithFormat:obfN(559), oldAngle, newAngle]);
}
...
- (void)panByX:(double)dx Y:(double)dy {
    ...
    vcam_ball_log([NSString stringWithFormat:obfN(560), nx, ny]);
}
...
- (void)zoomInTapped {
    ...
    vcam_ball_log([NSString stringWithFormat:obfN(561), nz]);
}
...
- (void)mirrorTapped {
    ...
    [self updateMirrorButtonVisual];
    vcam_ball_log([NSString stringWithFormat:obfN(564), newMirrored]);
}
...
- (void)openPickerForSlot:(NSInteger)slot {
    _pickerSlot = slot;
    vcam_ball_log([NSString stringWithFormat:obfN(566), (long)slot]);

    if (!NSClassFromString(obfN(567))) {
        vcam_ball_log(obfN(568));
        return;
    }
    dispatch_async(dispatch_get_main_queue(), ^{
...
- (BOOL)providerLooksLikeVideo:(NSItemProvider *)provider {
    if ([provider hasItemConformingToTypeIdentifier:obfN(569)]) return YES;
    if ([provider hasItemConformingToTypeIdentifier:obfN(570)]) return YES;
    for (NSString *tid in provider.registeredTypeIdentifiers) {
        if ([tid containsString:obfN(571)] || [tid containsString:obfN(334)] ||
            [tid containsString:obfN(572)] || [tid containsString:obfN(573)]) {
            return YES;
        }
    }
    return NO;
...
- (void)loadVideoFromProvider:(NSItemProvider *)provider candidates:(NSArray<NSString *> *)cands slot:(NSInteger)slot {
    ...
    [provider loadFileRepresentationForTypeIdentifier:tid
        ...
        if (!url) {
            vcam_ball_log([NSString stringWithFormat:obfN(574),
...
- (void)savePickedVideoAt:(NSURL *)srcUrl toSlot:(NSInteger)slot {
   ...
   withIntermediateDirectories:YES attributes:nil error:nil];
    ...
    vcam_ball_log([NSString stringWithFormat:obfN(575),
                   (long)slot, srcUrl.path, dest, ok ? obfN(425) : [copyErr localizedDescription]]);
    ...
    if (ok && slot == 0) {
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            vcam_ball_log([NSString stringWithFormat:obfN(576), dest]);
        });
    }
    ...
    if (![self providerLooksLikeVideo:provider]) {
        ...
        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.6 * NSEC_PER_SEC)),
            ...
            if ([strongSelf providerLooksLikeVideo:provider]) {
                vcam_ball_log(obfN(577));
                [strongSelf startVideoLoadFromProvider:provider slot:slot];
            } else {
                vcam_ball_log([NSString stringWithFormat:obfN(578),
...
- (void)startVideoLoadFromProvider:(NSItemProvider *)provider slot:(NSInteger)slot {
    ...
    if ([provider hasItemConformingToTypeIdentifier:obfN(569)]) {
        [cands addObject:obfN(569)];
    }
    for (NSString *tid in provider.registeredTypeIdentifiers) {
    ...
    if (cands.count == 0) [cands addObject:obfN(569)];
...
- (void)refreshLicenseStatus {
    if ([VCamNotify qvLv]) {
        _licenseStatusLabel.text = obfN(580);
...
- (void)licensePasteTapped {
    ...
    dispatch_async(dispatch_get_global_queue(QOS_CLASS_USER_INITIATED, 0), ^{
        ...
        if (s.length > 0) {
            ...
            vcam_ball_log([NSString stringWithFormat:obfN(584), (unsigned long)s.length]);
        }
    });
}
