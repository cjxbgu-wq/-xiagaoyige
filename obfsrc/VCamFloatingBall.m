...
static NSString *vcamGrouped16(NSString *raw16) {
    ...
    return [NSString stringWithFormat:obfN(405),
...
static void vcamIOSurfaceInit(void) {
    ...
    sVcamIOS.ok = 1;
    sVcamIOS.lock = (VcamIOSLockFn)dlsym(RTLD_DEFAULT, OBCS(406));
    sVcamIOS.unlock = (VcamIOSLockFn)dlsym(RTLD_DEFAULT, OBCS(407));
    sVcamIOS.base = (VcamIOSBaseAddrFn)dlsym(RTLD_DEFAULT, OBCS(408));
    sVcamIOS.width = (VcamIOSDimFn)dlsym(RTLD_DEFAULT, OBCS(409));
    sVcamIOS.height = (VcamIOSDimFn)dlsym(RTLD_DEFAULT, OBCS(410));
    sVcamIOS.stride = (VcamIOSDimFn)dlsym(RTLD_DEFAULT, OBCS(411));
    sVcamIOS.fmt = (VcamIOSFmtFn)dlsym(RTLD_DEFAULT, OBCS(412));
...
static NSString *vcamKnownLightName(int idx) {
    switch (idx) {
        case 0: return obfN(414);
        case 1: return obfN(415);
        case 2: return obfN(416);
        case 3: return obfN(417);
        case 4: return obfN(418);
        case 5: return obfN(419);
    }
    return obfN(420);
}
...
static mach_port_t vcamCARSPortForStrategy(int strategy) {
    ...
    if (strategy == kVcamPickStrategyBootstrap) {
        ...
        if (!luProbed) {
            luProbed = 1;
            lookUp = (VcamBootstrapLookUpFn)dlsym(RTLD_DEFAULT, OBCS(421));
        }
        if (!lookUp) return MACH_PORT_NULL;
        mach_port_t port = MACH_PORT_NULL;
        kern_return_t kr = lookUp(bootstrap_port, OBCS(422), &port);
    ...
    if (strategy == kVcamPickStrategySBSPort) {
        ...
        if (!sbsProbed) {
            sbsProbed = 1;
            sbsPort = (VcamSBSPortFn)dlsym(RTLD_DEFAULT, OBCS(423));
        }
...
static VcamIOSurfaceRef vcamCaptureDisplaySurfaceStrategy(int strategy) {
    ...
    if (!sVcamCARSFnProbed) {
        sVcamCARSFnProbed = 1;
        sVcamCARSFn = (VcamCARSCaptureFn)dlsym(RTLD_DEFAULT, OBCS(424));
        if (!sVcamCARSFn) {
            vcam_ball_log(obfN(425));
            return NULL;
        }
    }
...
static VcamUICreateScreenImageFn vcamUICreateScreenImage(void) {
    ...
    if (!probed) {
        probed = 1;
        fn = (VcamUICreateScreenImageFn)dlsym(RTLD_DEFAULT, OBCS(399));
        vcam_ball_log([NSString stringWithFormat:
            obfN(426), fn ? obfN(427) : obfN(428)]);
    }
    return fn;
}
...
- (void)createOverlayWindow {
    ...
    if (windowScene) {
        ...
        vcam_ball_log([NSString stringWithFormat:obfN(434), windowScene]);
    } else {
    ...
    // 点击手势
    UITapGestureRecognizer *tapGesture = [[UITapGestureRecognizer alloc] initWithTarget:self action:obfSEL(436)];
    [_ballView addGestureRecognizer:tapGesture];

    // 拖动手势
    UIPanGestureRecognizer *panGesture = [[UIPanGestureRecognizer alloc] initWithTarget:self action:obfSEL(437)];
...
- (VCamPanelButton *)makeButton:(NSString *)title frame:(CGRect)frame selector:(SEL)sel {
    ...
    [btn addTarget:self action:obfSEL(438) forControlEvents:UIControlEventTouchDown];
    [btn addTarget:self action:obfSEL(439) forControlEvents:UIControlEventTouchUpInside | UIControlEventTouchUpOutside | UIControlEventTouchCancel];
    return btn;
}
...
- (void)createPanel {
    ...
    [_tabControlBtn setTitle:obfN(440) forState:UIControlStateNormal];
    _tabControlBtn.titleLabel.font = [UIFont boldSystemFontOfSize:14];
    _tabControlBtn.layer.cornerRadius = 7;
    [_tabControlBtn addTarget:self action:obfSEL(441) forControlEvents:UIControlEventTouchUpInside];
    ...
    [_tabLightBtn setTitle:obfN(442) forState:UIControlStateNormal];
    _tabLightBtn.titleLabel.font = [UIFont boldSystemFontOfSize:14];
    _tabLightBtn.layer.cornerRadius = 7;
    [_tabLightBtn addTarget:self action:obfSEL(443) forControlEvents:UIControlEventTouchUpInside];
    ...
    [_tabSettingsBtn setTitle:obfN(444) forState:UIControlStateNormal];
    _tabSettingsBtn.titleLabel.font = [UIFont boldSystemFontOfSize:14];
    _tabSettingsBtn.layer.cornerRadius = 7;
    [_tabSettingsBtn addTarget:self action:obfSEL(445) forControlEvents:UIControlEventTouchUpInside];
    ...
    struct GridCell cells[4][4] = {
        { {CellIcon, nil, iconPlay, {2, 2, 2, 2}, obfSEL(446)},
          {CellSymbol, obfN(447), nil, {0, 0, 0, 0}, obfSEL(448)},
          {CellIcon, nil, iconRestore, {8, 8, 8, 8}, obfSEL(449)},
          {CellText, obfN(450), nil, {0, 0, 0, 0}, obfSEL(451)} },
        ...
        { {CellIcon, nil, iconRotate, {7, 7, 7, 7}, obfSEL(468)},
          {CellIcon, nil, iconMirror, {7, 7, 7, 7}, obfSEL(469)},
          {CellIcon, nil, iconReplace, {4, 4, 4, 4}, obfSEL(470)},
          {CellText, obfN(471), nil, {0, 0, 0, 0}, obfSEL(472)} },
    };
    for (int r = 0; r < 4; r++) {
        for (int c = 0; c < 4; c++) {
            ...
            if (cell.type == CellIcon) {
                ...
                if (sym) {
                    ...
                    btn = [self makeButton:obfN(473) frame:f selector:cell.action];
                    btn.titleLabel.font = [UIFont boldSystemFontOfSize:16];
                }
            } else {
    ...
    _pickColorBtn = [self makeButton:obfN(475)
    ...
    _lightColorLabel.text = obfN(477);
    ...
    NSString *rowTitles[5] = {obfN(478), obfN(479), obfN(480), obfN(481), obfN(482)};
    SEL rowActions[5] = {obfSEL(483), obfSEL(484),
                         obfSEL(485), obfSEL(486),
                         obfSEL(487)};
    int initVals[5] = {
    ...
    for (int i = 0; i < 5; i++) {
        ...
        val.text = [NSString stringWithFormat:obfN(488), v];
    ...
    VCamPanelButton *selectBtn = [self makeButton:obfN(489)
                                            frame:CGRectMake(pad, 0, contentW, rowH)
                                          selector:obfSEL(490)];
    selectBtn.titleLabel.font = [UIFont boldSystemFontOfSize:14];
    [_settingsPageView addSubview:selectBtn];

    VCamPanelButton *preset2 = [self makeButton:obfN(491)
    ...
    VCamPanelButton *preset3 = [self makeButton:obfN(493)
    ...
    VCamPanelButton *preset4 = [self makeButton:obfN(495)
    ...
    VCamPanelButton *licenseEntry = [self makeButton:obfN(497)
    ...
    UITapGestureRecognizer *codeTap = [[UITapGestureRecognizer alloc]
        initWithTarget:self action:obfSEL(501)];
    ...
    _licenseCodeHint.text = obfN(502);
    ...
    _licenseField.placeholder = obfN(504);
    ...
    VCamPanelButton *pasteBtn = [self makeButton:obfN(505)
    ...
    VCamPanelButton *activateBtn = [self makeButton:obfN(507)
    ...
    if ([VCamNotify plistLightEnabled]) {
        ...
        [_pickColorBtn setTitle:obfN(510) forState:UIControlStateNormal];
...
static NSString *vcamLightColorName(uint32_t c) {
    ...
    if (r > 200 && g < 80 && b < 80) return obfN(414);
    if (r < 80 && g > 200 && b < 80) return obfN(415);
    if (r < 80 && g < 80 && b > 200) return obfN(416);
    if (r > 200 && g > 200 && b < 80) return obfN(417);
    if (r < 80 && g > 200 && b > 200) return obfN(418);
    if (r > 200 && g < 80 && b > 200) return obfN(419);
    return obfN(514);
}
...
- (void)pickColorTapped {
    ...
    if (on) {
        ...
        // 打光功能已关 → 反相标题"开"
        [_pickColorBtn setTitle:obfN(475) forState:UIControlStateNormal];
...
- (void)showPickDot {
    ...
    CGFloat px = [pl[obfN(517)] doubleValue];
    CGFloat py = [pl[obfN(518)] doubleValue];
...
- (void)pickDotDragged:(UIPanGestureRecognizer *)g {
    ...
    if (g.state == UIGestureRecognizerStateEnded || g.state == UIGestureRecognizerStateCancelled) {
        ...
        vcam_ball_log([NSString stringWithFormat:obfN(521), c.x, c.y]);
    }
}
    ...
    if (!reconDumped) {
        ...
        if (classes) {
            ...
            for (unsigned int i = 0; i < clsCount; i++) {
                ...
                if ([name containsString:obfN(522)] || [name containsString:obfN(523)]) {
                    [hits addObject:name];
...
- (void)startColorPickup {
    ...
    gVcamPick.running = 1;
    NSDictionary *pl = [NSDictionary dictionaryWithContentsOfFile:ovf2()] ?: @{};
    double px = [pl[obfN(517)] doubleValue];
    double py = [pl[obfN(518)] doubleValue];
    if (px <= 0 || py <= 0) {
        CGRect sb = [UIScreen mainScreen].bounds;
...
- (void)colorPickTick {
    ...
    if (CFAbsoluteTimeGetCurrent() - lastTickLog > 1.0) {
        ...
        vcam_ball_log([NSString stringWithFormat:
            obfN(528),
            tickCount, _pickFastMode ? obfN(529) : obfN(530),
    ...
    if (wantFast != _pickFastMode) {
        ...
        vcam_ball_log([NSString stringWithFormat:
            obfN(531), wantFast ? obfN(530) : obfN(532),
            wantFast ? obfN(533) : obfN(534)]);
    }
    ...
    if ((uint32_t)detSlot != _lastDetectedColor) {  // ivar 复用: 存 slot 编号
        ...
        vcam_ball_log([NSString stringWithFormat:
            obfN(539),
            d, detName ?: obfN(540), detSlot, detCount, gVcamPick.avg]);
    }
}
...
- (void)updateColorPreview:(uint32_t)color {
    if (color == 0) {
        ...
        _lightColorLabel.text = [NSString stringWithFormat:obfN(541),
...
- (void)lightIntensityChanged:(UISlider *)s {
    _lightIntensityValue.text = [NSString stringWithFormat:obfN(488), (int)lroundf(s.value)];
    [self scheduleLightParamsFlush];
}
- (void)lightDiameterChanged:(UISlider *)s {
    _lightDiameterValue.text = [NSString stringWithFormat:obfN(488), (int)lroundf(s.value)];
    [self scheduleLightParamsFlush];
}
- (void)lightXChanged:(UISlider *)s {
    _lightXValue.text = [NSString stringWithFormat:obfN(488), (int)lroundf(s.value)];
    [self scheduleLightParamsFlush];
}
- (void)lightYChanged:(UISlider *)s {
    _lightYValue.text = [NSString stringWithFormat:obfN(488), (int)lroundf(s.value)];
    [self scheduleLightParamsFlush];
}
- (void)lightFeatherChanged:(UISlider *)s {
    _lightFeatherValue.text = [NSString stringWithFormat:obfN(488), (int)lroundf(s.value)];
    [self scheduleLightParamsFlush];
}
...
- (void)playPauseTapped {
    ...
    UIImageSymbolConfiguration *cfg =
        [UIImageSymbolConfiguration configurationWithPointSize:14
                                                       weight:UIImageSymbolWeightSemibold];
    UIImage *sym = [UIImage systemImageNamed:(_isPaused ? obfN(545) : obfN(462))
                            withConfiguration:cfg];
    [self.playPauseBtn setImage:sym forState:UIControlStateNormal];
    vcam_ball_log([NSString stringWithFormat:obfN(546), _isPaused ? obfN(162) : obfN(547)]);
}
...
- (void)toggleReplacementTapped {
    ...
    [self updateReplaceButtonVisual];
    vcam_ball_log([NSString stringWithFormat:obfN(549), newEnabled ? obfN(550) : obfN(551)]);
}
...
- (void)playSlot:(NSInteger)slot {
    ...
    if (![VCamNotify qvLv]) {
        vcam_ball_log([NSString stringWithFormat:obfN(557), (long)slot]);
        [self showLicensePage];
        return;
    }
    NSString *path = (slot == 1) ? obfN(137)
                                 : [NSString stringWithFormat:obfN(558), (long)slot];
    ...
    vcam_ball_log([NSString stringWithFormat:obfN(560), (long)slot, path]);
}
...
- (void)rotateRightTapped {
    ...
    vcam_ball_log([NSString stringWithFormat:obfN(561), oldAngle, newAngle]);
}
...
- (void)panByX:(double)dx Y:(double)dy {
    ...
    vcam_ball_log([NSString stringWithFormat:obfN(562), nx, ny]);
}
...
- (void)zoomOutTapped {
    double nz = vcamClamp([VCamNotify plistZoom] / vcamTZoomFactor(),
                          vcamTZoomMin(), vcamTZoomMax());
    [VCamNotify setPlistZoom:nz];
    vcam_ball_log([NSString stringWithFormat:obfN(564), nz]);
}
...
- (void)mirrorTapped {
    ...
    [self updateMirrorButtonVisual];
    vcam_ball_log([NSString stringWithFormat:obfN(566), newMirrored]);
}
...
- (void)openPickerForSlot:(NSInteger)slot {
    _pickerSlot = slot;
    vcam_ball_log([NSString stringWithFormat:obfN(568), (long)slot]);

    if (!NSClassFromString(obfN(569))) {
        vcam_ball_log(obfN(570));
        return;
    }
    dispatch_async(dispatch_get_main_queue(), ^{
...
- (BOOL)providerLooksLikeVideo:(NSItemProvider *)provider {
    ...
    if ([provider hasItemConformingToTypeIdentifier:obfN(572)]) return YES;
    for (NSString *tid in provider.registeredTypeIdentifiers) {
        if ([tid containsString:obfN(573)] || [tid containsString:obfN(334)] ||
            [tid containsString:obfN(574)] || [tid containsString:obfN(575)]) {
            return YES;
        }
    }
    return NO;
...
- (void)savePickedVideoAt:(NSURL *)srcUrl toSlot:(NSInteger)slot {
   ...
   withIntermediateDirectories:YES attributes:nil error:nil];
    ...
    vcam_ball_log([NSString stringWithFormat:obfN(577),
                   (long)slot, srcUrl.path, dest, ok ? obfN(427) : [copyErr localizedDescription]]);
    ...
    if (ok && slot == 0) {
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            vcam_ball_log([NSString stringWithFormat:obfN(578), dest]);
        });
    }
...
- (void)startVideoLoadFromProvider:(NSItemProvider *)provider slot:(NSInteger)slot {
    ...
    if ([provider hasItemConformingToTypeIdentifier:obfN(571)]) {
        [cands addObject:obfN(571)];
    }
    for (NSString *tid in provider.registeredTypeIdentifiers) {
    ...
    if (cands.count == 0) [cands addObject:obfN(571)];
...
- (void)refreshLicenseStatus {
    if ([VCamNotify qvLv]) {
        ...
        _licenseStatusLabel.text = obfN(583);
...
- (void)licensePasteTapped {
    ...
    dispatch_async(dispatch_get_global_queue(QOS_CLASS_USER_INITIATED, 0), ^{
        ...
        if (s.length > 0) {
            ...
            vcam_ball_log([NSString stringWithFormat:obfN(586), (unsigned long)s.length]);
        }
    });
}
...
- (void)licenseActivateTapped {
    ...
    if (ok) {
        ...
        _licenseStatusLabel.text = obfN(588);
        _licenseStatusLabel.textColor = [UIColor colorWithRed:0.95 green:0.40 blue:0.40 alpha:1.0];
