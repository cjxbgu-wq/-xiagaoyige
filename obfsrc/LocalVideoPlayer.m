...
- (instancetype)initWithCapacity:(NSUInteger)capacity {
    ...
    if (self) {
        ...
        _decodeQueue = dispatch_queue_create(OBCS(267), DISPATCH_QUEUE_SERIAL);
        _processingQueue = dispatch_queue_create(OBCS(268), DISPATCH_QUEUE_SERIAL);
        ...
        vcam_player_log([NSString stringWithFormat:obfN(269), (long)capacity]);
    }
    return self;
}
...
+ (VCamMediaType)detectMediaType:(NSString *)path {
    ...
    // 视频格式
    NSArray *videoExts = @[obfN(271), obfN(272), obfN(273), obfN(274), obfN(275), obfN(276)];
    if ([videoExts containsObject:ext]) return VCamMediaTypeVideo;
    // 图片格式
    NSArray *imageExts = @[obfN(277), obfN(278), obfN(279), obfN(280), obfN(281), obfN(282), obfN(283)];
...
static size_t vcam_decode_max_edge(void) {
    ...
    if (cached < 0) {
        @try {
            NSDictionary *d = [NSDictionary dictionaryWithContentsOfFile:obfN(0)];
            if (d && d[obfN(284)]) {
                NSInteger v = [d[obfN(284)] integerValue];
...
- (void)loadVideoAtPath:(NSString *)path completion:(void(^)(BOOL success, NSError *error))completion {
    if (!path || path.length == 0) {
        vcam_player_log(obfN(285));
        if (completion) completion(NO, [NSError errorWithDomain:obfN(286) code:1 userInfo:@{NSLocalizedDescriptionKey:obfN(287)}]);
        return;
    }
    ...
    if (![fm fileExistsAtPath:path]) {
        vcam_player_log([NSString stringWithFormat:obfN(288), path]);
        if (completion) completion(NO, [NSError errorWithDomain:obfN(286) code:2 userInfo:@{NSLocalizedDescriptionKey:obfN(289)}]);
        return;
    }
    ...
    if (type == VCamMediaTypeVideo) {
        vcam_player_log([NSString stringWithFormat:obfN(290), path]);
        [self loadVideoFile:path completion:completion];
    } else if (type == VCamMediaTypeImage) {
        vcam_player_log([NSString stringWithFormat:obfN(291), path]);
        [self loadImageFile:path completion:completion];
    } else {
        vcam_player_log([NSString stringWithFormat:obfN(292), path]);
        if (completion) completion(NO, [NSError errorWithDomain:obfN(286) code:3 userInfo:@{NSLocalizedDescriptionKey:obfN(293)}]);
    }
}

- (void)loadVideoFile:(NSString *)path completion:(void(^)(BOOL success, NSError *error))completion {
    vcam_player_log([NSString stringWithFormat:obfN(294), path]);
    ...
    if (![_assetPath isEqualToString:path] || !_reusableAsset) {
        ...
        vcam_player_log([NSString stringWithFormat:obfN(295), path]);
    }
    ...
    if (!_reusableTrack) {
        ...
        if (tracks.count == 0) {
            vcam_player_log(obfN(296));
            if (completion) completion(NO, [NSError errorWithDomain:obfN(286) code:4 userInfo:@{NSLocalizedDescriptionKey:obfN(297)}]);
            return;
        }
    ...
    vcam_player_log([NSString stringWithFormat:obfN(299),
...
- (void)rebuildReaderOnDecodeThread {
    ...
    if (readerErr || !_assetReader) {
        vcam_player_log([NSString stringWithFormat:obfN(300), readerErr]);
        return;
    }
    ...
    if (_resumeAtSeconds > 0.05 && _videoDuration > 0.3 &&
        ...
        vcam_player_log([NSString stringWithFormat:obfN(302), _resumeAtSeconds, _videoDuration]);
    }
    _resumeAtSeconds = 0;  // 一次性消费: 后续 rebuild(循环/重播/换源)都从头

    if (![_assetReader startReading]) {
        vcam_player_log([NSString stringWithFormat:obfN(303), _assetReader.error]);
        [_assetReader cancelReading];
        _assetReader = nil;
        _videoOutput = nil;
        return;
    ...
    vcam_player_log([NSString stringWithFormat:obfN(304),
...
- (void)loadImageFile:(NSString *)path completion:(void(^)(BOOL success, NSError *error))completion {
    vcam_player_log([NSString stringWithFormat:obfN(305), path]);
    ...
    if (!source) {
        vcam_player_log(obfN(306));
        if (completion) completion(NO, [NSError errorWithDomain:obfN(286) code:6 userInfo:@{NSLocalizedDescriptionKey:obfN(307)}]);
        return;
    }
    ...
    CFRelease(source);
    if (!cgImage) {
        vcam_player_log(obfN(308));
        if (completion) completion(NO, [NSError errorWithDomain:obfN(286) code:7 userInfo:@{NSLocalizedDescriptionKey:obfN(309)}]);
        return;
    }
    ...
    if (status != noErr || !pixelBuffer) {
        vcam_player_log([NSString stringWithFormat:obfN(310), (int)status]);
        CGImageRelease(cgImage);
        if (completion) completion(NO, [NSError errorWithDomain:obfN(286) code:8 userInfo:@{NSLocalizedDescriptionKey:obfN(311)}]);
        return;
    }
    ...
    _videoWidth = width;
    _videoHeight = height;
    vcam_player_log([NSString stringWithFormat:obfN(312), path, width, height]);
...
- (void)resetReaderForLoop {
    ...
    if (err || !_assetReader) {
        vcam_player_log([NSString stringWithFormat:obfN(314), err]);
        return;
    }
...
- (void)startDecodingThread {
    ...
    _decodeThread = [[NSThread alloc] initWithTarget:self selector:obfSEL(315) object:nil];
    _decodeThread.name = obfN(316);
...
- (void)startWatchingFile:(NSString *)path {
    ...
    if (!reloadListenerRegistered) {
        ...
        [[VCamNotify sharedInstance] registerForNotification:ovf1() callback:^(NSString *name) {
            [weakSelf reloadMedia];
        }];
        vcam_player_log([NSString stringWithFormat:obfN(323)]);
    }

    // 定时检查文件变化（每 2 秒）
    dispatch_queue_t watchQueue = dispatch_queue_create(OBCS(324), DISPATCH_QUEUE_SERIAL);
    ...
    vcam_player_log([NSString stringWithFormat:obfN(325), path]);
}
...
- (void)updateFileInfo {
    ...
    if (attrs) {
        ...
        vcam_player_log([NSString stringWithFormat:obfN(327),
...
- (void)checkFileChanges {
    ...
    if (!attrs) {
        // 文件被删除
        vcam_player_log([NSString stringWithFormat:obfN(328), _watchPath]);
        [self stopDecodingThread];
        [self clearFrameQueue];
        return;
    }
    ...
    if (newInode != _lastInode) {
        vcam_player_log([NSString stringWithFormat:obfN(329), _lastInode, newInode]);
        changed = YES;
    }
    if (newSize != _lastFileSize) {
        vcam_player_log([NSString stringWithFormat:obfN(330), _lastFileSize, newSize]);
        changed = YES;
    }
    if (fabs(newMtime - _lastMtime) > 1.0) {
        vcam_player_log([NSString stringWithFormat:obfN(331), _lastMtime, newMtime]);
        changed = YES;
    }
    ...
    if (changed) {
        ...
        _lastInode = newInode;
        _lastMtime = newMtime;
        vcam_player_log([NSString stringWithFormat:obfN(332), _watchPath]);
        [self reloadMedia];
    }
}
...
- (void)reloadMedia {
    ...
    vcam_player_log([NSString stringWithFormat:obfN(333),
                     (long)gen, path, _mediaType == VCamMediaTypeVideo ? obfN(334) : obfN(335)]);
    ...
    dispatch_async(_processingQueue, ^{
        ...
        if (gen != strongSelf.reloadGeneration) {
            vcam_player_log([NSString stringWithFormat:obfN(336),
        ...
        [strongSelf loadVideoAtPath:path completion:^(BOOL success, NSError *error) {
            if (success) {
                if (strongSelf.mediaType == VCamMediaTypeVideo) {
                    vcam_player_log([NSString stringWithFormat:obfN(337), (long)gen, path]);
                } else {
                    vcam_player_log([NSString stringWithFormat:obfN(338), (long)gen]);
                }
            } else {
                if (strongSelf.mediaType == VCamMediaTypeVideo) {
                    vcam_player_log([NSString stringWithFormat:obfN(339), (long)gen, error]);
                } else {
                    vcam_player_log([NSString stringWithFormat:obfN(340), (long)gen, error]);
                }
            }
        }];
    });
