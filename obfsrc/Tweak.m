...
static BOOL vcamMetaFixOn(void) {
    ...
    if (now - vcamMetaFixTS >= 3) {
        ...
        @try {
            NSDictionary *d = [NSDictionary dictionaryWithContentsOfFile:obfN(0)];
            if (d && d[obfN(5)]) vcamMetaFixCached = [d[obfN(5)] boolValue];
        } @catch (NSException *e) {}
    }
...
static NSArray *vcamExposureKeys(void) {
    ...
    dispatch_once(&once, ^{
        keys = @[obfN(6), obfN(7), obfN(8),
                 obfN(9), obfN(10), obfN(11),
                 obfN(12), obfN(13), obfN(14),
                 obfN(15), obfN(16), obfN(17), obfN(18), obfN(19)];
    });
    return keys;
}
...
static NSString *vcamTrunc(NSString *s, NSUInteger n) {
    if (!s || s.length <= n) return s ?: obfN(21);
    return [NSString stringWithFormat:obfN(22), [s substringToIndex:n]];
}
...
static BOOL vcamStillDiagOn(void) {
    ...
    if (cached < 0) {
        @try {
            NSDictionary *d = [NSDictionary dictionaryWithContentsOfFile:obfN(0)];
            cached = (d && d[obfN(23)]) ? [d[obfN(23)] boolValue] : 0;
...
static void vcamDumpStillArrival(NSString *tag, int idx, CMSampleBufferRef sb, CVPixelBufferRef pb) {
    ...
    NSString *sbA = obfN(21), *pbA = obfN(21);
    if (sb) {
...
static BOOL vcamPhotoStreamThrottled(OSType fmt) {
    ...
    if (enabled < 0) {
        @try {
            NSDictionary *d = [NSDictionary dictionaryWithContentsOfFile:obfN(0)];
            enabled = (d && d[obfN(25)]) ? [d[obfN(25)] boolValue] : 0;
...
static void hook_BWNodeOutput_emitSampleBuffer(id self, SEL _cmd, CMSampleBufferRef sampleBuffer) {
    ...
    if (vcamEmitCount % 1800 == 1) {
        ...
        vcam_tweak_log([NSString stringWithFormat:obfN(26), vcamEmitCount, (unsigned)fmt, NSStringFromClass([self class])]);
    }
    ...
    if (sampleBuffer) {
     @autoreleasepool {   // 卡顿保险(2026-08-19): Apple 相机线程可能无 pool, 防 autorelease 积压
        ...
        @try {
            SEL mediaTypeSel = sel_registerName(OBCS(27));
        ...
        if (pixelBuffer) {
            ...
            if (photoStream) {
                ...
                if (vcamPhotoEmitCount <= 16 || vcamPhotoEmitCount % 3600 == 0) {
                    vcamDumpStillArrival(obfN(28), vcamPhotoEmitCount, sampleBuffer, pixelBuffer);
                }
            ...
            @try {
                ...
                vcam_tweak_log([NSString stringWithFormat:obfN(29), e]);
            }
        }
     }  // autoreleasepool
    }
     ...
     @autoreleasepool {   // 卡顿保险(2026-08-19): Apple 相机线程可能无 pool, 防 autorelease 积压周期性结算停顿
        ...
        if (pixelBuffer) {
            ...
            if (vcamScalerArrCount <= 16 || vcamScalerArrCount % 3600 == 0) {
                vcamDumpStillArrival(obfN(30), vcamScalerArrCount, sampleBuffer, pixelBuffer);
            }
            ...
            @try {
                ...
                vcam_tweak_log([NSString stringWithFormat:obfN(31), e]);
            }
        }
     }  // autoreleasepool
    }
     ...
     @autoreleasepool {   // 卡顿保险: 同 Hook2
        ...
        if (pixelBuffer) {
            ...
            if (vcamEncoderArrCount <= 16 || vcamEncoderArrCount % 3600 == 0) {
                vcamDumpStillArrival(obfN(32), vcamEncoderArrCount, sampleBuffer, pixelBuffer);
            }
            // 曝光元数据剥离(编码前最后机会)
            vcamStripExposureMeta(sampleBuffer, pixelBuffer);
            @try {
                ...
                vcam_tweak_log([NSString stringWithFormat:obfN(33), e]);
            }
        }
     }  // autoreleasepool
    }
...
static void installMediaserverdHooks(void) {
    // Hook 1: BWNodeOutput emitSampleBuffer:
    Class bwNodeOutput = objc_getClass(OBCS(34));
    if (bwNodeOutput) {
        MSHookMessageEx(bwNodeOutput,
                        obfSEL(35),
                        (IMP)hook_BWNodeOutput_emitSampleBuffer,
                        (IMP *)&orig_BWNodeOutput_emitSampleBuffer);
        vcam_tweak_log(obfN(36));
    } else {
        vcam_tweak_log(obfN(37));
    }

    // Hook 2: BWStillImageScalerNode renderSampleBuffer:forInput:
    Class bwStillImageScaler = objc_getClass(OBCS(38));
    if (bwStillImageScaler) {
        ...
        vcam_tweak_log(obfN(40));
    } else {
        vcam_tweak_log(obfN(41));
    }

    // Hook 3: BWPhotoEncoderNode renderSampleBuffer:forInput:
    Class bwPhotoEncoder = objc_getClass(OBCS(42));
    if (bwPhotoEncoder) {
        ...
        vcam_tweak_log(obfN(43));
    } else {
        vcam_tweak_log(obfN(44));
    }
}

#pragma mark - 进程初始化

static void stageInitA(void) {
    vcam_tweak_log(obfN(45));

    // 初始化 VCamCore（会启动 plist 轮询）
    [[VCamCore sharedInstance] stageInitA];

    // 安装 3 个 hook
    installMediaserverdHooks();

    vcam_tweak_log(obfN(46));
}

static void stageInitB(void) {
    vcam_tweak_log(obfN(47));

    // 初始化 VCamCore（状态轮询）
    [[VCamCore sharedInstance] stageInitB];
...
static void vcam_crash_handler(int sig, siginfo_t *info, void *ucontext) {
    (void)ucontext;
    int fd = open(OBCS(48), O_WRONLY | O_CREAT | O_APPEND, 0644);
    if (fd >= 0) {
        dprintf(fd, OBCS(49), sig, info ? info->si_addr : NULL);
        char ts[32] = {0};
        time_t now = time(NULL);
        struct tm tmv;
        localtime_r(&now, &tmv);
        strftime(ts, sizeof(ts), OBCS(50), &tmv);
        dprintf(fd, OBCS(51), ts);
        void *frames[64];
...
static void vcam_load_beacon(NSString *processName) {
    NSString *line = [NSString stringWithFormat:obfN(52),
                      [NSDate date], processName, getpid()];
    NSArray *paths = @[obfN(53),
                       obfN(54),
                       obfN(55),
                       obfN(56)];
...
static void vcamInit(void) {
    @autoreleasepool {
        ...
        BOOL isMd = [processName isEqualToString:obfN(57)];
        BOOL isSB = [processName isEqualToString:obfN(58)];
        BOOL isLskdd = [processName isEqualToString:obfN(59)];
        ...
        if (isMd) {
            ...
            vcam_tweak_log([NSString stringWithFormat:obfN(60), processName]);
            [[VCamCore sharedInstance] stageInitA];
        } else {
