...
// Darwin 通知名（与原版保持一致）
NSString *ovf1(void);   // com.vcam.ios.media.reload
NSString *ovf0(void);   // com.vcam.ios.live.changed

// plist 路径（逆向确认）
NSString *ovf2(void);            // /var/mobile/Media/DCIM/vc.plist
NSString *ovf3(void);      // /var/mobile/vc.plist

typedef void(^VCamNotifyCallback)(NSString *name);

__attribute__((objc_runtime_name("Bt5"))) @interface VCamNotify : NSObject
...
//   单边被 Hook → 不一致 → 门禁关闭(VCamCore lq2)。
//   激活后永久有效(无月/年); 换设备 → 设备码变 → 密钥失效。
+ (NSString *)qvDc;                     // 16 hex 大写(设备码 raw)
+ (BOOL)qvLv;                         // 已激活(每次重验签+过期检查, 0.5s 节流)
+ (BOOL)qvLa:(NSString *)input;    // 激活(验签通过写 licBlob/expiryInfo)
+ (void)qvPd;                    // SB 侧发布 dcPub(md 互证用)
+ (BOOL)qvCc;                    // md 侧: dcPub 与本机一致
// 1.3.78 新增: 月卡过期支持
...
+ (double)qvTd:(NSUInteger)idx; // ×100 定点参数取值
+ (uint32_t)qvTi:(NSUInteger)idx;  // 整数参数取值(颜色/门限)
...
+ (void)qvSa;                     // App 进程采样器(Tweak.m 调)
+ (int)qvAp:(double)px               // 采样一拍(进程内 UICSI, 返色档)
                          Y:(double)py;
+ (void)qvNp:(int)slot;            // Darwin 通知色档上行(沙盒保底)
+ (void)qvSr;                      // SB 端中继(7 slot→总线)
+ (void)qvPc:(BOOL)on              // SB→App 配置下行(Darwin+state
                       X:(double)px                  // 沙盒安全; 坐标随 state 传递)
                       Y:(double)py;
+ (void)qvPb:(int)slot            // 写端: 采样器/SB/relay 双写
                      color:(uint32_t)color          // (color=标准纯色, 原版逻辑)
                      count:(int)cnt
                         avg:(uint32_t)avg;
+ (BOOL)qvRd:(uint32_t *)outColor // 读端: md 光轮询(≤1s 新鲜,
                      count:(int *)outCount;         // outColor 直接可打光)
+ (uint32_t)qvMk:(const uint8_t *)rgba  // 共享颜色匹配
                                    n:(int)n
